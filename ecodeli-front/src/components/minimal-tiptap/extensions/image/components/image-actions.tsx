...
const ActionItems: Array<{
  ...
  { key: 'onView', icon: <SizeIcon className="size-4" />, tooltip: 'voir' },
  { key: 'onDownload', icon: <DownloadIcon className="size-4" />, tooltip: 'download' },
  { key: 'onCopy', icon: <ClipboardCopyIcon className="size-4" />, tooltip: 'copyImage' },
  { key: 'onCopyLink', icon: <Link2Icon className="size-4" />, tooltip: 'copyLien', isLink: true }
]
    ...
    return (
      <ActionWrapper className={cn({ 'opacity-100': isOpen })}>
        {shouldMerge ? (
          <DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
            <DropdownMenuTrigger asChild>
              <ActionButton
                ...
                tooltip={t(`wysiwyg.image.ouvrirMenu`)}
              ...
              {filteredActions.map(({ key, icon, tooltip }) => (
                <DropdownMenuItem key={key} onClick={e => handleAction(e, actions[key])}>
                  <div className="flex flex-row items-center gap-2">
                    {icon}
                    <span>{t(`wysiwyg.image.${tooltip}`)}</span>
                  </div>
                </DropdownMenuItem>
              ))}
            </DropdownMenuContent>
          ...
          filteredActions.map(({ key, icon, tooltip }) => (
            <ActionButton key={key} icon={icon} tooltip={t(`wysiwyg.image.${tooltip}`)} onClick={e => handleAction(e, actions[key])} />
          ))
        )}
      </ActionWrapper>
    )
