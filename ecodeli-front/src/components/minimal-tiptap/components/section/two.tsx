...
export const SectionTwo: React.FC<SectionTwoProps> = ({
  ...
  const { t } = useTranslation(); // On récupère le hook de traduction

  const translatedActions = formatActions.map(action => ({
    ...action,
    label: t(`wysiwyg.format.${action.label}`)
  }));
  ...
  return (
    <ToolbarSection
      ...
      dropdownTooltip={t("wysiwyg.format.moreFormatting")}
